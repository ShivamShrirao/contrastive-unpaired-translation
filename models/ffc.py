...
class FFCResNetGenerator(nn.Module):
    def __init__(self, input_nc, output_nc, ngf=64, n_downsampling=3, n_blocks=9, norm_layer=nn.BatchNorm2d,
                 ...
                 init_conv_kwargs={}, downsample_conv_kwargs={}, resnet_conv_kwargs={'ratio_gin': 0.75, 'ratio_gout': 0.75},
        ...
        if len(layers) > 0:
            ...
            for layer_id, layer in enumerate(self.model):
                ...
                if layer_id in layers:
                    # print(f"{layer_id}: adding the output of {layer.__class__.__name__}, {len(feat)}")
                    if isinstance(feat, tuple):
                        feats.append(torch.cat(feat, dim=1))
                    else:
                        feats.append(feat)
                else:
                    # print(f"{layer_id}: skipping {layer.__class__.__name__}, {len(feat)}")
                    pass
